...
class SparkStreamAdapter(context: StreamingContext, config: Config) {

  // --------------------------------------------
  //  TODO: make remote system name configurable
  // --------------------------------------------
  ...
  def eventStream(id: String, host: String, port: Int, logName: String, fromSequenceNr: Long = 1L, storageLevel: StorageLevel = StorageLevel.MEMORY_ONLY): DStream[DurableEvent] =
    context.receiverStream(new DurableEventReceiver(id, ReplicationConnection(host, port), logName, fromSequenceNr, storageLevel, config))
}
    ...
    def receive = {
      case GetReplicationEndpointInfoSuccess(info) if !connected =>
        if (info.logNames.contains(logName)) {
          ...
          val source = ReplicationSource(info.endpointId, logName, sourceLogId, remoteAcceptor)
